...
void ler_senha(char *senha)
{
	...
	do
	{
		...
		{
			printf("Tamanho da senha. Por favor, insira uma senha válida com máximo com 20 características.\n");
		}
	} while (!senha_val);
}
...
void pesquisarUsuario(void)
{
	...
	else
	{
		...
		{
			if((strcmp(login->cpf, cpf) == 0) && (login->status == 'L'))
			{
				encotrado = 1;	
				exibeUsuario(login);
				getchar();
